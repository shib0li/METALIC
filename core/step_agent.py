...
class StepBanditAgent:
    ...
    def _init_models(self, state_dict):
        ...
        for i in range(self.opts):
            if i == 0:
                self.models.append(GPR(dim_cont=self.bandit.env.dim_S).to(self.device))
            else:
                self.models.append(GPR(dim_cont=self.bandit.env.dim_S+1).to(self.device))
            #
        #
    ...
    def _init_model_idx(self, idx):
        ...
        if idx == 0:
            new_model = GPR(dim_cont=self.bandit.env.dim_S).to(self.device)
        else:
            new_model = GPR(dim_cont=self.bandit.env.dim_S+1).to(self.device)
        #
    ...
    def replay(self, idx, gam=0.9):
        ...
        if idx == 0:
            ...
            X = torch.tensor(np.hstack([loss, states, actions])).to(self.device)
            y = torch.tensor(rewards).to(self.device)
...
#         #self.models[idx] = GPR().to(self.device)
...
#         optimizer.step(closure)
    ...
    def _step_rnd(self, state, idx, loss=None, seed=None):
        #print('rnd')
    ...
    def _step_ucb(self, state, idx, loss=None, c=1.0):
        #print('ucb')
        ...
        if idx > 0:
            ...
            Xte = torch.hstack([vloss, states, actions])
...
#         print(ytr.shape)
    ...
    def _step_ts(self, state, idx, loss=None, ns=20):
        #print('ts')
...
#         argmax_aid = torch.argmax(samples_mean).item()
    ...
    def _step_mean(self, state, idx, loss=None):
...
#         print(ytr.shape)
    ...
    def _sample_action_pathway(self, state, method, update):
        ...
        for idx in range(self.opts):
            ...
            if method == 'ts':
                action = self._step_ts(state, idx, loss)
            elif method == 'ucb':
                action = self._step_ucb(state, idx, loss)
            elif method == 'rnd':
                action = self._step_rnd(state, idx, loss)
            elif method == 'mean':
                action = self._step_mean(state, idx, loss)
            else:
