...
#         return u_xx + u_yy - f # Poisson problem residual
    ...
    def get_loss_pinn(self):
        ...
        #mse_f = (u_y - u_xx - uu*(1-uu)).square().mean()
        
        mse_f = (u_y-self.rau*uu*(1-uu)).square().mean()
...
#             xcgpinn_error_u_total = np.linalg.norm(xcgpinn_u_vals.flatten()-xcgpinn_u_pred.flatten(),1)/np.linalg.norm(xcgpinn_u_vals.flatten(),1)
            
            return xcgpinn_error_u_total


class PINN_Client_Reaction:
...
# pinn_client = PINN_Client_Reaction(
#     num_adam=1,
#     int_adam=2000,
#     lr_adam=1e-3,
#     num_lbfgs=1,
