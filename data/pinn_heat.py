...
def heat_solver(x, t, param):
    ...
    term1 = a*np.exp(-np.pi*np.pi*t)*np.cos(np.pi*x)
    term2 = b*np.exp(-4*np.pi*np.pi*t)*np.cos(2*np.pi*x)
    term3 = c*np.exp(4*t-4)*np.cosh(2*x)
    term4 = d*np.exp(t-1)*np.sinh(x)
...
def gen_heat_data(
    ...
    bounds = np.vstack((
        np.hstack((x_left, t.reshape([-1,1]))),
        np.hstack((x_right, t.reshape([-1,1]))),
        np.hstack((x.reshape([-1,1]), t_left)),
        np.hstack((x.reshape([-1,1]), t_right))
    ))
    ...
    colls1 = generate_with_bounds(N=1000, lb=lb1, ub=ub1, method='lhs', seed=1)
    colls2 = generate_with_bounds(N=1000, lb=lb2, ub=ub2, method='lhs', seed=2)
    ...
    U = heat_solver(mesh_x.flatten(), mesh_t.flatten(), param)
    #print(U.shape)
    ...
    task_data['ub1'] = heat_solver(bound1[:,0], bound1[:,1], param).squeeze()

    task_data['xb2'] = bound2[:,0]
    task_data['yb2'] = bound2[:,1]
    task_data['ub2'] = heat_solver(bound2[:,0], bound2[:,1], param).squeeze()

    task_data['xf1'] = colls1[:,0]
    task_data['yf1'] = colls1[:,1]
    task_data['ff1'] = heat_solver(colls1[:,0], colls1[:,1], param).squeeze()

    task_data['xf2'] = colls2[:,0]
    task_data['yf2'] = colls2[:,1]
    task_data['ff2'] = heat_solver(colls2[:,0], colls2[:,1], param).squeeze()

    task_data['xi1'] = interface[:,0]
    task_data['yi1'] = interface[:,1]
    task_data['fi1'] = heat_solver(interface[:,0], interface[:,1], param).squeeze()

    task_data['x1'] = S1[:,0]
    task_data['y1'] = S1[:,1]
    task_data['u1'] = heat_solver(S1[:,0], S1[:,1], param).squeeze()

    task_data['x2'] = S2[:,0]
    task_data['y2'] = S2[:,1]
    task_data['u2'] = heat_solver(S2[:,0], S2[:,1], param).squeeze()
...
class PINN_Client_Heat:
    ...
    def __init__(self, 
        ...
        self.reset(param=np.array([1.0, 0.6, 0.3, 0.1]))
...
#             raise Exception('Error, no interface given...')
    ...
    def reset(self, param=None):
        ...
        if param is None:
            param=np.array([1.0, 0.6, 0.3, 0.1])
